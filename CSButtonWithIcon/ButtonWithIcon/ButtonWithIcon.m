...
- (void)initIconLabel {
    _iconlabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, _baseimage.frame.size.width*0.3, _baseimage.frame.size.width*0.3)];
    _iconlabel.center = CGPointMake(_baseimage.frame.origin.x+_baseimage.frame.size.width-_iconlabel.frame.size.width/2, _baseimage.frame.origin.y+_iconlabel.frame.size.height/2);
    [_iconlabel setText:@""];
...
- (void)setIconLabelMode:(BOOL)isCircularBtn {
    if (isCircularBtn) {
        _iconlabel.frame = CGRectMake(0, 0, _baseimage.frame.size.width*0.3, _baseimage.frame.size.width*0.3);
        _iconlabel.center = CGPointMake(_baseimage.frame.origin.x+_baseimage.frame.size.width-_iconlabel.frame.size.width/2, _baseimage.frame.origin.y+_iconlabel.frame.size.height/2);
        _iconlabel.layer.cornerRadius = _iconlabel.frame.size.height/2;//如果圆角为一半，则可以截成圆形
    }
    else {
